...
palette=sns.cubehelix_palette(5, start=2, rot=0, dark=0, light=.95, reverse=False)
sns.lmplot(x="oil_price", y="share_price_scaled",hue="year", col="name",ci=None,col_wrap=3, data=all_data13, order=1,palette=palette,size=4).set(ylim=(0, 1))
...
#plt.plot(x_train['oil_price'], regr.predict(x_train), color='black', linewidth=3)
#plt.plot(x_test['oil_price'],regr.predict(x_test),  color='black', linewidth=3)
plt.xlabel("oil_price")
plt.ylabel("share_price")
plt.title("Linear Regression (Data 01)")
#plt.title("Linear Regression (Data 01) 1 feature")
plt.suptitle('Error ((actual-predicted)^2/actual) % : '+str(error1))
plt.legend((plt_train, plt_test),("train data", "test data"))
plt.savefig('graph/oilngas01_linear_regression.png')
#plt.savefig('graph/oilngas01_linear_regression_01f.png')
plt.show() 
...
for f in range(x_train.shape[1]):
    print("Feature %s (%f)" % (indices[f], coeff1[f]))

f, (ax1) = plt.subplots(1, 1, figsize=(25, 20), sharex=True)
sns.barplot(indices, coeff1, palette="BrBG", ax=ax1)
ax1.set_ylabel("Co-efficient")
ax1.set_title("Co-efficient (Data 01) Linear Regression")
ax1.figure.savefig('graph/oilngas01_linear_regression_coefficient.png')
